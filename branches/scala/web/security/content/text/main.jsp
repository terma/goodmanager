...
<html>
    ...
    <body>
        ...
        <p>
            К <a href="<login:link value="/security/content/main.jsp"/>">главной</a>,
            <a href="<login:link value="/security/content/text/add.jsp"/>">добавить</a> текст
        </p>
        <% if (texts.isEmpty()) { %>
        ...
        <% } else { %>
            <% for (final EntityText text : texts) { %>
                <p>
                    <a href="<login:link value="<%= "/security/content/text/edit.jsp?textId=" + text.id %>"/>"><%= text.name %></a><br>
                    <%= text.description %>
                </p>
            <% } %>
        <% } %>
