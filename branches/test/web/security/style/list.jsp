...
<html>
    ...
    <body>
        <p>
            Стили всего <%= styles.size() %>, вернуться на <a href="<login:link value="/security/main.jsp"/>">главную</a>
            создать <a href="<login:link value="/security/style/add.jsp"/>">новый</a> стиль
        </p>
        <ul>
            <% for (final EntityStyle style : styles) { %>
                <li>
                    <%= LogicStyle.getHtml(style, style.name) %>, <a href="<login:link value="<%= "/security/style/edit.jsp?styleid=" + style.id %>"/>">редактировать</a>
                    <div class="info">
