...
<html>
    ...
    <body>
        ...
        <p>
            ...
            <a href="/security/product/currency/main.jsp"/>">валютам</a>
        </p>
        <table border="0" cellpadding="0" cellspacing="0">
            <tr>
                <td valign="top">
                    Добавить <a href="/security/product/categoryadd.jsp"/>">категорию</a>
                ...
                <td valign="top" style="padding-left: 20px">
                    ...
                    <% } else { %>
                        <p>
                            ...
                            <a href="<%= "/security/product/productadd.jsp?categoryId=" + selectCategory.id %>"/>">продукт</a>,
                            или добавить <a href="<%= "/security/product/categoryadd.jsp?parentcategoryid=" + selectCategory.id %>"/>">подкатегорию</a>
                        </p>
                        <% if (products.isEmpty()) { %>
                            В этой категории к сожалению нет продуктов. Но можно
                            <a href="<%= "/security/product/productadd.jsp?categoryId=" + selectCategory.id %>"/>">добавить</a>
                        <% } else { %>
                            <% for (final EntityProduct product : products) { %>
                                <a href="<%= "/security/product/productdelete.jsp?productId=" + product.id %>"/>"><img
                                        style="vertical-align: middle;" src="/image/delete.gif" alt="Удалить"
                                        width="15" height="15" border="0"></a>
                                <%= product.name %>
                                <div class="productInfo">
                                    <a href="<%= "/security/product/productedit.jsp?productid=" + product.id %>"/>">редактировать</a>
                                    , добавить <a href="<%= "/security/product/productpriceadd.jsp?productid=" + product.id %>"/>">цену</a><br>
