...
<html>
    ...
    <body>
        <p>
            ...
            <a href="/security/mail/main.jsp"/>">почта</a>,
            <a href="/security/mail/server/add.jsp"/>">добавить</a> сервер,
        </p>
            ...
            <% for (final EntityServer server : servers) { %>
                <li>
                    <% if (server.type == EntityServerType.INBOX) { %>
                        ...
                        <% if (server.type == EntityServerType.INBOX) { %>
                            <a href="<%= "/security/mail/server/rule/add.jsp?serverId=" + server.serverId %>"/>">добавить</a> получателя
                            , <a href="<%= "/security/mail/server/deleteconfirm.jsp?serverId=" + server.serverId %>"/>">удалить</a>
                        <% } %>
