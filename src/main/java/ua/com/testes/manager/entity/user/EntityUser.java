...
@Entity(name = "users")
public final class EntityUser implements Serializable {
    ...
    private List<EntityTask> dependTasks = new ArrayList<EntityTask>();

    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @OneToMany(mappedBy = "owner", cascade = {javax.persistence.CascadeType.ALL})
    private List<EntityTask> ownerTasks = new ArrayList<EntityTask>();
    ...
    @OneToMany(mappedBy = "user", cascade = {javax.persistence.CascadeType.ALL})
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    private List<EntityMail> mails = new ArrayList<EntityMail>();
